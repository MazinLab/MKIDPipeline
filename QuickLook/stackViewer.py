...
#dataPath = '/mnt/data0/darknessCalFiles/imageStacks/'
...
class DarkQuick(QtGui.QMainWindow):
    ...
    def initWindows(self):
        ...
        #self.finalStackWindow = FinalStackWindow(self)
        self.plotWindows = []
    ...
    def createWidgets(self):
        ...
        self.label_startTstamp = QtGui.QLabel(str(self.timeStamps[0]))
        self.label_endTstamp = QtGui.QLabel(str(self.timeStamps[-1]))
        self.lineEdit_currentTstamp = QtGui.QLineEdit(str(self.timeStamps[self.currentImageIndex]))

        self.label_dither = QtGui.QLabel("Dither #: ")
        self.lineEdit_ditherPos = QtGui.QLineEdit(str(self.ditherArray[self.currentImageIndex]))
        self.label_centroid = QtGui.QLabel("Centroid (x,y): ")
        self.lineEdit_centX = QtGui.QLineEdit(str(self.centXArray[self.currentImageIndex]))
        self.lineEdit_centY = QtGui.QLineEdit(str(self.centYArray[self.currentImageIndex]))
    ...
    def arrangeMainFrame(self):
        ...
        infoBox = layoutBox('H',[1,self.label_dither,self.lineEdit_ditherPos,1,self.label_centroid,
                self.lineEdit_centX,self.lineEdit_centY,1])

        mainBox = layoutBox('V',[canvasBox,incrementControlsBox,infoBox])
    ...
    def createMenu(self):        
        ...
        #finalStackAction = createAction(self,"View Final Stack",slot=self.finalStackWindow.show)
        ...
        #add additional menu items for opening stacked frame, params table, etc
    ...
    def connectControls(self):
        ...
        self.connect(self.lineEdit_ditherPos,QtCore.SIGNAL('editingFinished()'),self.jumpToDither)

        self.connect(self.imageParamsWindow,QtCore.SIGNAL('newImageParams()'), self.getObsImage)
    ...
    def loadImageStack(self):
        if (not os.path.exists(self.h5Path)):
            msg='file does not exist: %s'%self.h5Path
            if verbose:
                print msg
            raise Exception(msg)
        self.file = tables.openFile(self.h5Path, mode='r')

        self.stackParams = self.file.getNode('/imageStack/params').read()

        self.imageStack = np.array(self.file.getNode('/imageStack/rawImgs').read(),dtype=float)
        self.timeStamps = np.array(self.file.getNode('/imageStack/timestamps').read())
        self.ditherArray = np.array(self.file.getNode('/imageStack/dithers').read())
        self.hotArray = np.array(self.file.getNode('/imageStack/hpms').read())
        self.coldArray = np.array(self.file.getNode('/imageStack/cpms').read())
        self.deadArray = np.array(self.file.getNode('/imageStack/dpms').read())
        self.aperArray = np.array(self.file.getNode('/imageStack/ams').read())
        self.roughXArray = np.array(self.file.getNode('/imageStack/roughX').read())
        self.roughYArray = np.array(self.file.getNode('/imageStack/roughY').read())
        self.fineXArray = np.array(self.file.getNode('/imageStack/fineX').read())
        self.fineYArray = np.array(self.file.getNode('/imageStack/fineY').read())
        self.centXArray = np.array(self.file.getNode('/imageStack/centX').read())
        self.centYArray = np.array(self.file.getNode('/imageStack/centY').read())
        self.dark = np.array(self.file.getNode('/imageStack/dark').read())
        self.flat = np.array(self.file.getNode('/imageStack/flat').read())
        self.final = np.array(self.file.getNode('/imageStack/finalImg').read())
    ...
    def getObsImage(self):
        self.lineEdit_currentTstamp.setText(str(self.timeStamps[self.currentImageIndex]))
        self.lineEdit_ditherPos.setText(str(self.ditherArray[self.currentImageIndex]))
        self.lineEdit_centX.setText(str(self.centXArray[self.currentImageIndex]))
        self.lineEdit_centY.setText(str(self.centYArray[self.currentImageIndex]))

        paramsDict = self.imageParamsWindow.getParams()
        stackOptions=paramsDict['stackOptions']
        self.image = np.copy(self.imageStack[self.currentImageIndex])
        
        if 'dark' in stackOptions and stackOptions['dark']==True:
            self.image-=self.dark
        if 'hpm' in stackOptions and stackOptions['hpm']==True:
            hpm = [self.hotArray[self.currentImageIndex]]
            self.image[hpm]=np.nan
        if 'cpm' in stackOptions and stackOptions['cpm']==True:
            self.image[self.image<stackOptions['coldCut']]=np.nan
    ...
    def jumpToTstamp(self):
        ...
        if desiredTstamp not in self.timeStamps:
            print 'Warning: requested time stamp is outside available range'
        else:
            self.currentImageIndex = np.where(self.timeStamps==desiredTstamp)[0][0]
            self.getObsImage()

    def jumpToDither(self):
        desiredDither = int(self.lineEdit_ditherPos.text())
        if desiredDither not in self.ditherArray:
            print 'Warning: requested dither number is outside available range'
        else:
            self.currentImageIndex = np.where(self.ditherArray==desiredDither)[0][0]
            self.getObsImage()
    ...
    def plotArray(self,*args,**kwargs):
        centerX = self.centXArray[self.currentImageIndex]
        centerY = self.centYArray[self.currentImageIndex]
        self.arrayImageWidget.defineCentroid(centerX,centerY)
...
#        timesChoiceBox = layoutBox('H',[self.checkbox_trackTimes,self.timesGroup])
    ...
    def plotLightCurve(self,getRaw=False):
        ...
        self.axes.plot(self.parent.timeStamps, self.lightCurve)
...
#class FinalStackWindow(ModelessWindow):
        #A separate window to simply display final image stack from H5.
        #Needs signal functionality to send click command up to parent,
        #so pixel clicked in final stack is highlighted in raw images.
        #Can maybe use ArrayImageWidget, but needs centroid stuff fixed,
        #which currently assumes parent is main window with that info.

class ImageParamsWindow(ModelessWindow):
    def __init__(self,parent=None,hoverCall=None):
        super(ImageParamsWindow,self).__init__(parent=parent)
        self.parent=parent
    ...
    def initUI(self):
        ...
        self.lineEdit_coldCut = QtGui.QLineEdit(str(self.parent.stackParams['coldCut'][0]))
        
        coldCutBox = layoutBox('H',[self.label_coldCut,self.lineEdit_coldCut])
        plotArrayBox = layoutBox('V',[self.combobox_cmap,self.checkbox_applyDark,self.checkbox_applyFlat,
                self.checkbox_applyHPM,self.checkbox_applyCPM,coldCutBox])
    ...
    def connectChangeSignals(self):
        self.connect(self.checkbox_applyDark,QtCore.SIGNAL('stateChanged(int)'), self.emitNewParams)
        self.connect(self.checkbox_applyFlat,QtCore.SIGNAL('stateChanged(int)'), self.emitNewParams)
        self.connect(self.checkbox_applyHPM,QtCore.SIGNAL('stateChanged(int)'), self.emitNewParams)
        self.connect(self.checkbox_applyCPM,QtCore.SIGNAL('stateChanged(int)'), self.emitNewParams)
        self.connect(self.lineEdit_coldCut,QtCore.SIGNAL('editingFinished()'), self.emitNewParams)
        self.connect(self.combobox_cmap,QtCore.SIGNAL('currentIndexChanged(int)'), self.emitNewParams)
    ...
    def getParams(self):
        ...
        stackOptionDict['dark']=self.checkbox_applyDark.isChecked()
        stackOptionDict['flat']=self.checkbox_applyFlat.isChecked()
        stackOptionDict['hpm']=self.checkbox_applyHPM.isChecked()
        stackOptionDict['cpm']=self.checkbox_applyCPM.isChecked()
        stackOptionDict['coldCut']=int(self.lineEdit_coldCut.text())

        outDict = {}
        outDict['plotParams'] = plotParamsDict
        outDict['stackOptions'] = stackOptionDict
        return outDict

    def emitNewParams(self):
        self.emit(QtCore.SIGNAL('newImageParams()'))
...
class ArrayImageWidget(QtGui.QWidget):
    ...
    def plotArray(self,image,normNSigma=3,title='',stackOptions={},**kwargs):
        ...
        if not 'vmax' in kwargs:
            goodImage = self.image[np.isfinite(self.image)]
        ...
        self.handleMatshow = self.axes.matshow(self.image,**kwargs)
    ...
    def defineCentroid(self, centerX, centerY):
        self.centroid = tuple([centerX, centerY])

    def drawCentroid(self):
        if self.centroid != None:
            patch = matplotlib.patches.Circle(xy=self.centroid,radius=0.5,
