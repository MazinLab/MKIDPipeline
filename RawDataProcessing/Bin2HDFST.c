...
void ParseBeamMapFile(char *BeamFile, uint32_t **BeamMap, uint32_t **BeamFlag, beamMapCoords *resIDSortedCoords)
{
    ...
    do {
        ...
        //printf("%d %d %d %d\n", resid, flag, x, y);
        BeamMap[x][y] = resid;
        resIDSortedCoords->x = x;
        resIDSortedCoords->y = y;
        resIDSortedCoords++;
        if(flag>0)
            BeamFlag[x][y] = 1;
        else
...
int main(int argc, char *argv[])
{
    ...
    photon p1;
    beamMapCoords *resIDSortedCoords; //holds beammap coordinates sorted by resID
    beamMapCoords curCoords;
    hsize_t *tablePositions;
    ...
    {"ResID", "Time","Wavelength","Spec Weight","Noise Weight"};
    ...
    resIDSortedCoords = (beamMapCoords*)malloc(beamCols * beamRows * sizeof(beamMapCoords));
    tablePositions = (hsize_t*)calloc(beamCols * beamRows, sizeof(hsize_t));
    ...
    ParseBeamMapFile(BeamFile,BeamMap,BeamFlag,resIDSortedCoords);
    ...
    H5TBmake_table( "Photon Data", file_id, "/Photons/PhotonTable", NFIELD, 0, dst_size, field_names, dst_offset, field_type,chunk_size, fill_data, compress, &p1);
        
    // allocate memory for photon lists 
    for(i=0; i < beamCols; i++) {
		for(j=0; j < beamRows; j++) {
			if( BeamMap[i][j] == 0 ) 
            {
    ...
    for(i=0; i < nFiles; i++) {
        ...
        for(j=0; j<beamCols*beamRows; j++) {
            curCoords = resIDSortedCoords[j];
			if( BeamMap[curCoords.x][curCoords.y] == beamMapInitVal ) continue;
			if( ptablect[curCoords.x][curCoords.y] == 0 ) continue;    
			//printf("%s %ld\n", ResIdString[j][k], ptablect[j][k]);
			printf("%d %d %ld %ld\n", j, k, ptablect[curCoords.x][curCoords.y], tablePositions[j]); fflush(stdout);
			H5TBinsert_record(file_id, "/Photons/PhotonTable", tablePositions[j], ptablect[curCoords.x][curCoords.y], dst_size, dst_offset, dst_sizes, ptable[curCoords.x][curCoords.y] );
			nPhot +=  ptablect[curCoords.x][curCoords.y];
            for(k=j; k<beamCols*beamRows; k++)
                tablePositions[k] += ptablect[curCoords.x][curCoords.y];
