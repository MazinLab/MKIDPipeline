...
class ObsFile:
    ...
    def loadFile(self, fileName,verbose=False):
        ...
        #get photon table
        self.photonTable = self.file.root.Photons.PhotonTable
...
#        return plFile
    ...
    def getPixelPhotonList(self, xCoord, yCoord, firstSec=0, integrationTime= -1, wvlRange=None, isWvl=True):
        ...
        if wvlRange is None and integrationTime==-1:
            ...
            if integrationTime == -1:
                photonList = self.photonTable.read_where('(ResID == resID) & (Wavelength >= startWvl) & (Wavelength < endWvl)')
            else:
                photonList = self.photonTable.read_where('(ResID == resID) & (Time > startTime) & (Time < endTime) & (Wavelength >= startWvl) & (Wavelength < endWvl)')
    ...
    def getPixelCountImage(self, firstSec=0, integrationTime= -1, wvlRange=None, applyWeight=True, applyTPFWeight=True, applyTimeMask=True, scaleByEffInt=False):
        ...
        startTs = firstSec*10.**6
        endTs = startTs + integrationTime*10.**6
        photonList = self.photonTable.read_where('((Time > startTs) & (Time < endTs))') 
        for xCoord in range(self.nXPix):
            for yCoord in range(self.nYPix):
                effIntTimes[xCoord, yCoord] = integrationTime
                countImage[xCoord, yCoord] = np.shape(np.where(photonList['ResID']==self.beamImage[xCoord,yCoord])[0])[0]
