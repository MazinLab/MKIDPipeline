...
int main(int argc, char *argv[])
{
    ...
    int FirstFile, nFiles,mapflag, roachNum;
    long fSize, rd, j, k;
    struct stat st;
    FILE *fp;
    uint64_t **data, *dSize;
    ...
    {
        sprintf(tname, "/Photons/%d", i);
        gid_roaches[i] = H5Gcreate2(file_id, tname, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    ...
    for(i=0; i < BEAM_COLS; i++) {
		for(j=0; j < BEAM_ROWS; j++) {
            ...
            roachNum = BeamMap[i][j]/1000;
			sprintf(tname,"/Photons/%d/%d", roachNum, BeamMap[i][j]);			
