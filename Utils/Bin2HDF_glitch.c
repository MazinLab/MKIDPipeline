...
uint64_t prevHdrTs[10] = {0}; //most recent base Ts for each roach - used for fixing glitches
...
void AddPacket(char *packet, uint64_t l, hid_t file_id, size_t dst_size, size_t dst_offset[NFIELD], size_t dst_sizes[NFIELD], int FirstFile, uint32_t BeamMap[BEAM_COLS][BEAM_ROWS], photon *p, uint64_t *nPhot)
{
    ...
    // same for prev timestamp list
    if( prevHdrTs[roach] == 0 ) prevHdrTs[roach] = (uint64_t) hdr->timestamp;
    if((hdr->timestamp - prevHdrTs[roach]) >= 2000)
        basetime -= 2000; //fix glitch caused by extra PPS
