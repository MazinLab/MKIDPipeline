...
class MainWindow(QMainWindow):
    ...
    def load_beam_map(self):
        filename, _ = QFileDialog.getOpenFileName(self, 'Select One File', '/mnt/data0/Darkness/20180522/Beammap/',
                                                  filter='*.txt')
    ...
    def getDarkFrame(self):
        ...
        for ii in range(darkIntTime):
            try:
                darkFrameFilename = self.fileListRaw[
                    np.where(self.timeStampList == (self.spinbox_darkStart.value() + ii))[0][0]]
            except:
                pass
            else:
                darkFrame[ii] = np.transpose(
                    np.reshape(np.fromfile(open(darkFrameFilename, mode='rb'), dtype=np.uint16),
                               (self.nCol, self.nRow)))
    ...
    def updateLogLabel(self, IMG_fileExists=True):
        ...
        text = 'img timestamp:\n' + datetime.datetime.fromtimestamp(timestamp).strftime(
            '%Y-%m-%d %H:%M:%S') + '\n\nLogfile time:\n' + datetime.datetime.fromtimestamp(logLabelTimestamp).strftime(
            '%Y-%m-%d %H:%M:%S\n') + '\n' + labelFilename[:-4] + '\n' + fin.read()
        self.label_log.setText(text)
        fin.close()
    ...
    def create_main_frame(self):
        ...
        self.dpi = 100
        self.fig = Figure(figsize=(5.0, 10.0), dpi=self.dpi,
                          tight_layout=True)  # define the figure, set the max size (inches) and resolution. Overall window size is set with QMainWindow parameter.
