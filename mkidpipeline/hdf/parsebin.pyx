...
cdef extern from "binlib.h":
    long parsebin(const char *fName, unsigned long max_len, int* baseline, int* wavelength, unsigned long* timestamp, unsigned int* ycoord, unsigned int* xcoord, unsigned int* roach)
...
def parse(file, n=0):
    ...
    ret = np.array([baselne[:r],phase[:r],tstamp[:r],y[:r],x[:r]], dtype=['baseline', int),'wavelength', int),'timestamp', float64),('y', int), ('x', int)])
