...
def aperture(startpx, startpy, radius, nRows, nCols):
    ...
    for x in allx:
        for y in ally:
            if np.abs(x - startpx) ** 2 + np.abs(y - startpy) ** 2 <= r ** 2 and 0 <= y < nRows and 0 <= x < nCols:
                mask[y, x] = 1.
    return mask
...
def rotateShiftImage(obsArray, degree, xShift, yShift):
    ...
    rotatedIm = ndimage.rotate(obsArray, degree, order=0, cval=-1, reshape=False).astype(float)
    rotatedIm[rotatedIm < 0] = np.nan
...
def correlation(p, fjac=None, refImage=None, imToAlign=None, err=None, returnCorrelation=False):
    ...
    # the figure of merit is calculated by multiplying each pixel in the two images and
    # dividing by the maximum of the two pixels squared.
...
def alignImages(refImage, imToAlign, parameterGuess, parameterLowerLimit, parameterUpperLimit,
                parameterMinStep=[0.05, 2, 2], model=correlation):
    """
    Runs mpfit.py
    Inputs:
    ...
    for k in range(len(parameterGuess)):
        ...
        par = {'n': k, 'value': parameterGuess[k], 'limits': [parameterLowerLimit[k], parameterUpperLimit[k]],
               'limited': [lowLimit, upLimit], 'fixed': fixGuess, 'step': parMinStep}
        parinfo.append(par)
    quiet = True
    ...
    m = mpfit.mpfit(correlation, functkw=fa, parinfo=parinfo, maxiter=1000, xtol=10 ** (-20), ftol=10 ** (-20),
                    gtol=10 ** (-30), quiet=quiet)
...
def calculateRotationAngle(refImage=None, imToAlign=None, intTime=None):
    ...
    """
    if refImage is None and imToAlign is None and intTime is not None:
        timeInterval = intTime
    elif refImage is not None and imToAlign is not None:
        ...
        return
    return 360.0 * timeInterval / secondsPerDay
