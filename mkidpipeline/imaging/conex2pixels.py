...
def get_transforms(ditherfile, datadir, wvl_start=None, wvl_stop=None, fwhm_guess=3.0, fit_power=1, CONEX_ERROR=0.0001, plot=False):
    ...
    #these don't get used, photutils.centroids does not return an error and skimate.tform does not accept any input errors
    ...
    if plot:
        ...
        for index, image in enumerate(debug_images[:-1]):
            ...
            axs[i,j].add_patch(Rectangle((source_est[index][0]- (box_size / 2), source_est[index][1] - (box_size / 2)), box_size, box_size,
            ...
            axs[i,j].plot(pixel_positions[index][0], pixel_positions[index][1], color='blue', marker=marker, ms=ms, mew=mew)
            axs[i,j].set_title('Red + = Estimate, Blue + = Centroid for Dither Pos %i'%index)
            if (index+1)%axis == 0 and (index+1)!=len(obs_files):
                    i=i+1
                    j=0
            elif (index+1)!=len(obs_files):
                    j=j+1
        plt.show()
...
if __name__ == '__main__':
    ...
    xform_con2pix, xform_pix2con=get_transforms(ditherfile, datadir, fwhm_guess=3.0, wvl_start=900, wvl_stop=1200, fit_power=1)